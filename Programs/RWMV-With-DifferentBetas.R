...
data <- rbind(ant1, ant2, ant3, ant4, camel1, camel2, camel3, camel4,
              ckjm, ivy1, ivy2, ivy3, jedit1, jedit2, jedit3, jedit4, jedit5,
...
RMVPredictions <- rbind(as.numeric(PredsLR),as.numeric(PredsKNN), as.numeric(PredsSVM),
                        as.numeric(PredsDT), as.numeric(PredsNB))
...
for(i in c(1:nrow(testData))) {
  if(TestDepVar[i]==1 && RWMVHIELFinalPredictions[i]==0){
    CountLoC <- CountLoC + testData$loc[i]
  }
}
SavedCost <- CountLoC/TotalLoC
SavedCost*100
##############################################
# 3. Percent of Non-Perfect Cleans
##############################################
RemainingEdits <- (TotalLoC-CountLoC)/TotalLoC
RemainingEdits*100
##############################################
