...
data <- rbind(ant1, ant2, ant3, ant4, ant5, camel1, camel2, camel3, camel4, jedit1, jedit2,
              jedit3, jedit4, jedit5, log4j1, log4j2, log4j3, poi1, poi2, 
              poi3, poi4, redaktor, tomcat, velocity1,
              velocity2, xalan1, xalan2, xalan3, xalan4, xerces1, xerces2, xerces3)
...
for (i in c(1:10)) {
  ...
  # train(trainSetNB[,-20],trainSetNB$defective,'nb',trControl=trainControl(method='cv',number=10))
  #train(trainSetNB[,-20],trainSetNB$defective,'nb',trControl=trainControl(method='cv',number=10))
  #model_NB
  ########################################################
  # Prediction using Test Set
  ...
  #confusionMatrix(Predict_NB, testDataNB$defective)
...
for (i in c(1:10)) {
  ...
  #model_NN <- neuralnet(trainSet$defective ~ ., data=trainSet, algorithm = "rprop+",
  #                      hidden=c(15), threshold=0.5, rep=5, stepmax = 1e+06) 
  #model_NN <- neuralnet(trainSet$defective ~ ., data=trainSet, hidden=2, err.fct="ce", linear.output=FALSE)
...
#RMVPredictions <- rbind(as.numeric(PredsLR),as.numeric(PredsDT),as.numeric(PredsKNN),as.numeric(PredsNB),as.numeric(PredsSVM),as.numeric(PredsNN))
RMVPredictions <- rbind(as.numeric(PredsLR))
RWMVWeights <- replicate(10,1)
tempProbs <- replicate(10,0)
tempPreds <- replicate(10,0)
...
# Ploting
##############################################
diverse10 <- c(0.9375,0.9375,0.9375,0.9375,0.9375,0.9375,0.9375,0.9375,0.9375)
diverse20 <- c(0.9352,0.9315,0.9283,0.9283,0.9283,0.9283,0.9283,0.9283,0.9283)
diverse30 <- c(0.9283,0.9246,0.9236,0.9161,0.9161,0.9161,0.9161,0.9161,0.9161)
diverse40 <- c(0.931,0.9198,0.9198,0.9198,0.9198,0.9198,0.9198,0.9198,0.9198)
diverse50 <- c(0.9315,0.9315,0.9315,0.9315,0.9315,0.9315,0.9315,0.9315,0.9315)
diverse60 <- c(0.9551,0.9387,0.9387,0.9387,0.9387,0.9387,0.9387,0.9387,0.9387)

png(file = "Synapse.jpg")
# Plot the bar chart.
plot(diverse10, type = "o",col = "red", xlab = "β Values", ylab = "F-Measure Values",
     ylim=c(0.9,0.98), lwd = 2)
